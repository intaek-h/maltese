...
export async function reportPunServerAction(args: {
  punPublicKey: string;
}): Promise<
  | { success: false; message: string }
  | { success: true; message: string; data: { reportCount: number } }
> {
  ...
  try {
    const res = await fetchMutation(api.reports.reportPun, {
      authorKey,
      ipHash,
    ...
    if (res.isDuplicateRequest) {
      return {
        ...
        data: { reportCount: res.reportCount },
      };
    }
    ...
    console.error("Error while calling reportPun:", error);
